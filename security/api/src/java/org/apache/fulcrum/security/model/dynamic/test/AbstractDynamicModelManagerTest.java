...
public abstract class AbstractDynamicModelManagerTest extends BaseUnitTest {
	...
	public void testRetrieveingUsersByGroup() throws Exception {
		...
		((DynamicModelManager) securityService.getModelManager()).grant(user,
				group);
		assertTrue(((DynamicGroup) group).getUsers().contains(user));
	...
	public void testAddRemoveDelegate() throws Exception {
		DynamicUser borris = (DynamicUser) userManager
				.getUserInstance(USERNAME_BORRIS);
		userManager.addUser(borris, "mc");
		DynamicUser sam = (DynamicUser) userManager
				.getUserInstance(USERNAME_SAM);
		...
		DynamicUser borrisLoaded = (DynamicUser) userManager
				.getUser(USERNAME_BORRIS);
